...
View(Boston)
head(Boston)
# Boston dataset shows information about the Housing Values in Suburbs of Boston. 
# Crime rate, residential land zoned, non-retail business zones, etc. 
...
```{r}
p_scatter <- ggplot(Boston, aes(x=lstat, y=medv)) + geom_point(size = 1) + labs(x ="Lower status of the population (%)", y= "Median value of owner-occupied homes in $1000s")
p_scatter
...
# Intercept = 34.553
# lstat = -0.95
# We can conclude that there is a negative correlation, aka the lower the housing price, the more sizable the lower
# economic population
```
...
```{r}
ggplot(data = Boston, aes(x = y_pred, y=medv)) +geom_point() +
  xlab("Predicted Y Values") + ylab("True Boston Housing Values")
```
...
```{r}
pred_dat <- data.frame(
  lstat = seq(0,40, length.out = 1000))
```
...
```{r}
p_scatter <- ggplot(Boston, aes(x=Boston$lstat, y=Boston$medv)) + geom_point(size = 1) + geom_line(data= pred_dat, aes(x = y_pred_new, y = lstat)) + 
labs(x ="Lower status of the population (%)", y= "Median value of owner-occupied homes in $1000s") 
p_scatter 
# Don't forget to use the aes() for geom_line
...
p_scatter <- ggplot(Boston, aes(x=lstat, y=medv)) + 
  geom_point(size = 1) + 
  geom_line(data= pred_dat, aes(x = y_pred_new, y = lstat)) + 
  geom_ribbon(aes(x = lstat, ymin= lower, ymax = dataframe12$upper), data=dataframe12) +
  labs(x ="Lower status of the population (%)", y= "Median value of owner-occupied homes in $1000s") 
p_scatter
...
mse <- function(y_true, y_pred) {
  mse <- sum((y_true - y_pred) ^2)/length(y_pred)
  return(mse)
}
```
...
```{r}
?sample()
boston_master <- mutate(Boston, sample(splits))
names(boston_master)[names(boston_master) == "sample(splits)"] <- "splits"
...
```{r}
model_1_mse_train <- mse(Boston$medv, predict(model_1))
...
```{r}
model_2 <- lm(medv ~ lstat, boston_valid)
summary(model_2)
model_1_mse_valid <- mse(Boston$medv, predict(model_2))
predict( , newdata =model_2)
